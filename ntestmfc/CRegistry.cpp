...
BOOL CRegistry::OpenKey(Keys hKey, LPCTSTR szKey)
{
...
BOOL CRegistry::CreateKey(Keys hKey, LPCTSTR szKey)
{
	LSTATUS lstatus = RegCreateKeyEx((HKEY)HKEY_CLASSES_ROOT, (LPCTSTR)szKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &m_hKey, NULL);
...
BOOL CRegistry::SetValue(LPCTSTR lpValueName, LPCTSTR lpData)
{
	...
	{
		::RegFlushKey(m_hKey);		//明确将注册表信息记录入硬盘（开销会较大）
		return TRUE;
	}
