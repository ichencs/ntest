...
// 		{
			...
			struct Accessor
			{
				static __forceinline void* Get(TreeNode* root, uint8_t index)
				{
					if (!root)
					{
						return 0;
				...
				static __forceinline void Set(TreeNode*& root, uint8_t index, void* value, PushOnlyAllocator<TreeNode>& allocator)
				{
					if (!root)
					{
...
// 		}
		...
		public:
			...
			void Set(uint8_t index, T* value, Allocator& allocator)
			{
