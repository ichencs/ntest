...
// 	DateTime ConvertTMToDateTime(tm* timeinfo, int milliseconds)
// 	{
...
//         dt.filetime = (uint64_t)timer * 1000 + milliseconds;
// 		dt.totalMilliseconds = (uint64_t)timer * 1000 + milliseconds;
// 		return dt;
// 	}
// 
...
// #endif
    ...
    DateTime DateTime::FromFileTime(uint64_t filetime)
	{
		ULARGE_INTEGER largeInteger;
	...
	DateTime DateTime::Forward(uint64_t milliseconds)
	{
		return FromFileTime(filetime+milliseconds*10000);
	}

	DateTime DateTime::Backward(uint64_t milliseconds)
	{
		return FromFileTime(filetime-milliseconds*10000);
	}
