...
// 	template<>struct POD<uint64_t>{static const bool Result=true;};
...
***********************************************************************/
	...
	struct DateTime
	{
		...
		static DateTime		FromFileTime(uint64_t filetime);
		...
		DateTime			Forward(uint64_t milliseconds);
		/// <summary>Move Backward.</summary>
		/// <returns>The moved time.</returns>
		/// <param name="milliseconds">The delta in milliseconds.</param>
		DateTime			Backward(uint64_t milliseconds);
