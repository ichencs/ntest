...
// 		CloseHandle(handle);
	...
	WString System::DnsHotsNameToComputer(WString dnsHost)
	{
		wchar_t computer[100] = { 0 };
		unsigned long length = sizeof(computer)/sizeof(*computer);
		BOOL bres = DnsHostnameToComputerNameExW(dnsHost.Buffer(),computer,&length);
		return computer;
	}
